...
fun inferTypesIn(element: FuncInferenceContextOwner): FuncInferenceResult = recursionGuard(element, memoize = false) {
    FuncInferenceContext(element.project).infer(element)
}
...
data class FuncInferenceResult(
    ...
    override fun getExprTy(expr: FuncExpression?): FuncTy = exprTypes[expr] ?: FuncTyUnknown
...
class FuncInferenceContext(val project: Project) : FuncInferenceData {
    ...
    fun isTypeInferred(expression: FuncExpression): Boolean = exprTypes.containsKey(expression)
