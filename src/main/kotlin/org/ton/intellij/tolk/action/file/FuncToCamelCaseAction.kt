...
class TolkToCamelCaseAction : AnAction() {
    ...
    override fun update(e: AnActionEvent) {
        ...
        e.presentation.isEnabledAndVisible = file is TolkFile
    }

    override fun actionPerformed(e: AnActionEvent) {
        val project = e.project ?: return
        val file = e.getData(CommonDataKeys.PSI_FILE) as? TolkFile ?: return

        WriteCommandAction.runWriteCommandAction(project) {
            PsiTreeUtil.collectElementsOfType(file, TolkFunction::class.java).forEach {
                val name = it.name
                if (name != null) {
                    val newName = transformString(name, 1)
                    if (newName != name) {
                        it.identifier.replace(TolkPsiFactory[project].createIdentifier(newName))
                    }
                }
            }
            PsiTreeUtil.collectElementsOfType(file, TolkReferenceExpression::class.java).forEach {
                val name = it.name
                if (name != null) {
                    val newName = transformString(name, 1)
                    if (newName != name) {
