            ...
            override fun visitExpression(expression: FuncExpression) {
                ...
                val valueText = buildString {
                    if (value is FuncIntValue &&
                        expression is FuncLiteralExpression &&
...
class FuncComputeConstantExpressionFix(element: PsiElement, private val valueText: String) :
    LocalQuickFixAndIntentionActionOnPsiElement(element),
    LocalQuickFix {
    override fun getFamilyName(): String = InspectionBundle.message("fix.constant.expression.family.name")

    override fun getText(): String = if (valueText.length < MAX_RESULT_LENGTH_TO_DISPLAY) {
        InspectionBundle.message("fix.constant.expression.name", valueText)
    } else {
        InspectionBundle.message("fix.constant.expression.name.short")
    }
