...
class FuncToTolkConverterV1 {
    ...
    fun convertFunction(psiElement: FuncFunction, out: FuncToTolkState) {
        ...
        asmBody != null && !isImpure
        psiElement.inlineKeyword != null
        psiElement.inlineRefKeyword != null
        psiElement.methodIdDefinition?.lparen == null
        ...
        out.createEmptyFork(psiElement.firstChild)
        val sName = out.createEmptyFork(nameIdentifier)
        val sGenerics = psiElement.forallKeyword?.let { out.createEmptyFork(it) }
        psiElement.functionParameterList.firstOrNull()?.let { out.createEmptyFork(it) }
