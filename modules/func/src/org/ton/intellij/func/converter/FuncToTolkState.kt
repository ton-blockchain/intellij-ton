...
class FuncToTolkState(
    ...
    fun merge(vararg forksAndDelimiters: FuncToTolkState?) {
        ...
        for (fork in forksAndDelimiters) {
            ...
            if (forkedPsi != null) {
                minStartOffset = minOf(minStartOffset, forkedPsi.textRange.startOffset)
                maxEndOffset = maxOf(maxEndOffset, forkedPsi.textRange.endOffset)
            }
        }
    ...
    fun justSkipPsi(psiElement: PsiElement, withTrailingNl: Boolean = true) {
        if (psiElement.textRange.startOffset > lastEndOffset) {
            tolkSource.append(funcSource.substring(lastEndOffset, psiElement.textRange.startOffset))
        }
        lastEndOffset = psiElement.textRange.endOffset
