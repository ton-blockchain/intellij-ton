...
sealed interface TolkValue : TolkTyProvider {
}
...
data class TolkTupleValue(val values: List<TolkValue?>) : TolkValue {
    override fun getTolkTy(): TolkTy {
        return TolkTyTuple(values.map { it?.getTolkTy() ?: TolkTyUnknown })
    }
}

data class TolkTensorValue(val values: List<TolkValue?>) : TolkValue {
    override fun getTolkTy(): TolkTy {
        return TolkTyTensor(values.map { it?.getTolkTy() ?: TolkTyUnknown })
    }
}
