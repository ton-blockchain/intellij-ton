...
abstract class TolkBraceTypedHandler(private val handler: BraceHandler) : TypedHandlerDelegate() {
...
abstract class TolkBraceBackspaceHandler(private val handler: BraceHandler) : TolkEnableableBackspaceHandlerDelegate() {

    override fun deleting(c: Char, file: PsiFile, editor: Editor): Boolean {
        if (c == handler.opening.char && file is TolkFile) {
            val offset = editor.caretModel.offset
