...
class TonWslToolchain(
        val wslPath: WslPath
) : TonToolchainBase(wslPath.distribution.getWindowsPathWithFix(wslPath.linuxPath).toPath()) {
    ...
    override fun hasToncliExecutable(exec: String): Boolean =
            distribution.getWindowsPath(pathToToncliExecutable(exec)).isFile()
